...
from modelopen.models_utils import weights_init, print_network
...
class LeFF(nn.Module):
    def __init__(self, dim=32, hidden_dim=128, act_layer=nn.GELU, drop = 0., use_eca=False):
        ...
        self.dwconv = nn.Conv2d(hidden_dim*2,hidden_dim*2,groups=hidden_dim*2,kernel_size=3,stride=1,padding=1)  #fast leff
...
class BasictransganLayer(nn.Module):
...
class transgan(nn.Module):
    def __init__(self, img_size=256, in_chans=13, dd_in=15,
                 embed_dim=16, depths=[2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2],
                 num_heads=[1, 2, 4, 8, 16, 32, 32, 16, 8, 4, 2],
                 win_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, token_projection='linear', token_mlp='leff',
                 dowsample=Downsample, upsample=Upsample, shift_flag=True, modulator=False,**kwargs):
        super().__init__()
        ...
        # Encoder
        self.encoderlayer_0 = BasictransganLayer(dim=embed_dim,
        ...
        self.encoderlayer_1 = BasictransganLayer(dim=embed_dim * 2,
        ...
        self.encoderlayer_2 = BasictransganLayer(dim=embed_dim * 4,
        ...
        self.encoderlayer_3 = BasictransganLayer(dim=embed_dim * 8,
        ...
        self.encoderlayer_4 = BasictransganLayer(dim=embed_dim * 16,
                                                ...
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                shift_flag=shift_flag)
        # self.dowsample_4 = dowsample(embed_dim*16, embed_dim*32)
        self.dowsample_4 = dowsample(embed_dim * 16, embed_dim * 16)

        # Bottleneck
        # self.conv = BasicUformerLayer(dim=embed_dim*32,
        #                     output_dim=embed_dim*32,
        # input_resolution = (img_size // (2 ** 5),
        #                     img_size // (2 ** 5)),
        self.conv = BasictransganLayer(dim=embed_dim * 16,
                                      output_dim=embed_dim * 16,
                                      input_resolution=(img_size // (2 ** 4),
                                                        img_size // (2 ** 4)),
                                      depth=depths[5],
        ...
        self.decoderlayer_0 = BasictransganLayer(dim=embed_dim * 32,
                                                output_dim=embed_dim * 32,
                                                input_resolution=(img_size // (2 ** 4),
                                                                  img_size // (2 ** 4)),
                                                depth=depths[6],
        ...
        self.decoderlayer_1 = BasictransganLayer(dim=embed_dim * 16,
                                                output_dim=embed_dim * 16,
                                                input_resolution=(img_size // (2 ** 3),
                                                                  img_size // (2 ** 3)),
                                                depth=depths[7],
        ...
        self.decoderlayer_2 = BasictransganLayer(dim=embed_dim * 8,
                                                output_dim=embed_dim * 8,
                                                input_resolution=(img_size // 2 ** 2,
                                                                  img_size // 2 ** 2),
                                                depth=depths[8],
                                                ...
                                                shift_flag=shift_flag,
                                                modulator=modulator)
        self.upsample_3 = upsample(embed_dim * 8, embed_dim * 2)
        self.decoderlayer_3 = BasictransganLayer(dim=embed_dim * 4,
                                                output_dim=embed_dim * 4,
                                                input_resolution=(img_size // 2,
                                                                  img_size // 2),
                                                depth=depths[9],
                                                ...
                                                shift_flag=shift_flag,
                                                modulator=modulator)
        self.upsample_4 = upsample(embed_dim * 4, embed_dim)
        self.decoderlayer_4 = BasictransganLayer(dim=embed_dim * 2,
                                                output_dim=embed_dim * 2,
                                                input_resolution=(img_size,
                                                                  img_size),
                                                depth=depths[10],
    ...
    def forward(self, x, mask=None):
        ...
        conv4 = self.encoderlayer_4(pool3, mask=mask)
        pool4 = self.dowsample_4(conv4)

        # Bottleneck
        conv5 = self.conv(pool4, mask=mask)

        # Decoder

        up0 = self.upsample_0(conv5)
        deconv0 = torch.cat([up0, conv4], -1)
        deconv0 = self.decoderlayer_0(deconv0, mask=mask)

        up1 = self.upsample_1(deconv0)
        deconv1 = torch.cat([up1, conv3], -1)
        deconv1 = self.decoderlayer_1(deconv1, mask=mask)

        up2 = self.upsample_2(deconv1)
        deconv2 = torch.cat([up2, conv2], -1)
        deconv2 = self.decoderlayer_2(deconv2, mask=mask)

        up3 = self.upsample_3(deconv2)
        deconv3 = torch.cat([up3, conv1], -1)
        deconv3 = self.decoderlayer_3(deconv3, mask=mask)

        up4 = self.upsample_4(deconv3)
        deconv4 = torch.cat([up4, conv0], -1)
        deconv4 = self.decoderlayer_4(deconv4, mask=mask)

        # Output Projection
        y = self.output_proj(deconv4)
        # return y
        return x[:, :13, :, :] + y
    ...
    def flops(self):
        ...
        flops += self.encoderlayer_4.flops() + self.dowsample_4.flops(self.reso // 2 ** 4, self.reso // 2 ** 4)
        ...
        flops += self.upsample_4.flops(self.reso // 2, self.reso // 2) + self.decoderlayer_4.flops()
...
class Generator(nn.Module):
    def __init__(self, in_ch, out_ch, gpu_ids):
        ...
        input_size = 256
        self.gen = nn.Sequential(OrderedDict([('gen', transgan(img_size=input_size, in_chans = out_ch, dd_in =in_ch,
                                                              embed_dim=32,depths=[2, 2, 2, 2, 2,2,2, 2, 2, 2, 2],
